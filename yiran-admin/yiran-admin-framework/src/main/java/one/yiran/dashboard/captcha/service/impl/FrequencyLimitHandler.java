...
public interface FrequencyLimitHandler {
    ...
    class DefaultLimitHandler implements FrequencyLimitHandler {
        ...
        public DefaultLimitHandler(Properties config) {
            this.config = config;
        }
        ...
        public ResponseModel validateVerify(CaptchaVO d) {
            ...
            String v = DashboardCacheService.getCacheService().get(key);
            if (Objects.isNull(v)) {
                DashboardCacheService.getCacheService().set(key, "1", 60);
                v = "1";
            }
            DashboardCacheService.getCacheService().increment(key, 1);
