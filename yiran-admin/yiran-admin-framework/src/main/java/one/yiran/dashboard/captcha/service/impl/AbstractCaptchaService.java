...
public abstract class AbstractCaptchaService implements CaptchaService {
    ...
    public void init(final Properties config) {
        ...
        waterMark = config.getProperty(Const.CAPTCHA_WATER_MARK, "");
...
//        }
    ...
    protected DashboardCacheService getCacheService(String cacheType) {
        return DashboardCacheService.getCacheService();
    }
    ...
    protected void afterValidateFail(CaptchaVO data) {
        if (limitHandler != null) {
            ...
            DashboardCacheService cs = getCacheService(cacheType);
            if (!cs.exists(fails)) {
                cs.set(fails, "1", 60);
            }
        ...
        try {
            if (waterMarkFontStr.toLowerCase().endsWith(".ttf") || waterMarkFontStr.toLowerCase().endsWith(".ttc")
                ...
                this.waterMarkFont = Font.createFont(Font.TRUETYPE_FONT,
                        LoadFileUtil.loadStream("/fonts/" + waterMarkFontStr))
